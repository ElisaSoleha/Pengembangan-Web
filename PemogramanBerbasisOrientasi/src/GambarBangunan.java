...
class GambarBangunan extends JPanel{

    private boolean isNight = true; // variabel untuk menentukan apakah malam atau tidak
    private boolean isRaining = false; // variabel untuk menentukan apakah hujan atau tidak
    private boolean isBuildingAnimationInProgress = true;
    private int buildingProgress = 0; // variabel untuk melacak kemajuan pembangunan
   public static void main(String[] args) {
        
        JFrame Pandangan = new JFrame ("GambarBagunan");
        GambarBangunan Gambar = new GambarBangunan();
        Pandangan.getContentPane().add(Gambar);
        Pandangan.pack();
        Pandangan.setSize(new Dimension(800, 500));
        Pandangan.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        Pandangan.setVisible(true);

        new Thread(() -> {
            while (Gambar.isBuildingAnimationInProgress) {
                try {
                    Thread.sleep(1000); // delay 1000 ms (1 detik)
                    Gambar.repaint(); // repaint panel
        ...
        new Thread(() -> {
            while (true) {
                try {
                    ...
                    Gambar.toggleRain(); // toggle status hujan
                    Pandangan.repaint(); // repaint panel
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }
        ...
        // Thread untuk mengatur perubahan malam menjadi siang
        new Thread(() -> {
            while (true) {
                try {
                    Thread.sleep(5000); // delay 5000 ms (5 detik)
                    Gambar.toggleDayNight(); // toggle status malam/siang
                    Pandangan.repaint(); // repaint panel
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
        ...
        new Thread(() -> {
            while (Gambar.isBuildingAnimationInProgress) {
                try {
                    ...
                    if (Gambar.buildingProgress < 100) {
                        Gambar.buildingProgress += 10; // meningkatkan kemajuan pembangunan
                    } else {
                        Gambar.stopBuildingAnimation(); // jika selesai, hentikan animasi
                    }
    ...
    public void paintComponent(Graphics g){
        ...
        Dua.fill(Langit);//Dua.draw(Langit);
